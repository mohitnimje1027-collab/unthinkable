...
const createComplaint = async ({ user_id, category, description, photo_url }) => {
  ...
  await pool.query(
    `INSERT INTO complaint_history (complaint_id, status, note, changed_by)
     VALUES ($1, 'Open', 'Complaint submitted', $2)`,
...
const updateComplaintStatus = async (id, { status, priority, note, changed_by }) => {
  ...
  if (status) {
    await pool.query(
      `INSERT INTO complaint_history (complaint_id, status, note, changed_by) VALUES ($1, $2, $3, $4)`,
...
const markOverdue = async (id, is_overdue) => {
  const res = await pool.query(
    `UPDATE complaints SET is_overdue = $1, updated_at = NOW() WHERE id = $2 RETURNING *`,
    [is_overdue, id]
  );
  return res.rows[0];
};
