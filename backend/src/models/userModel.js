...
const createUser = async ({ name, email, password, role = 'resident', flat_no }) => {
  ...
  const res = await pool.query(
    'INSERT INTO users (name, email, password_hash, role, flat_no) VALUES ($1, $2, $3, $4, $5) RETURNING id, name, email, role, flat_no, created_at',
...
const findUserById = async (id) => {
  const res = await pool.query(
    'SELECT id, name, email, role, flat_no, created_at FROM users WHERE id = $1',
    [id]
  );
  return res.rows[0];
};
